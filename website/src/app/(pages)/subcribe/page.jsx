...
export default function SubscribePage() {
  ...
  const monthlyPrice = useMemo(
    () => calculatePrice(planKey, duration),
    [planKey, duration]
  );
  const originalMonthly = useMemo(() => basePrices[planKey] || 0, [planKey]);
  const total = useMemo(
    () => calculateTotalPrice(planKey, duration),
    [planKey, duration]
  );
  const savings = useMemo(
    () => calculateSavings(planKey, duration),
    [planKey, duration]
  );
              ...
              return (
                <button
                  ...
                  className={`relative rounded-[20px] bg-white p-5 text-left shadow-[0_8px_30px_rgba(16,24,40,0.08)] ring-1 ring-slate-100 transition-colors ${
                    selected
                      ? "outline  outline-indigo-500/80 bg-indigo-50/40"
                      : "hover:bg-slate-50"
                  }`}
                >
                  ...
                  <div className="mt-2 text-lg font-extrabold text-[#5A57FF]">
                    {perMonth} DA/month
                  </div>
                  {optDiscounted && (
          ...
          <div className="mt-10 rounded-[24px] bg-white p-6 shadow-[0_8px_30px_rgba(16,24,40,0.08)] ring-1 ring-slate-100">
            <div className="grid gap-6 lg:grid-cols-2">
                  ...
                  {promoMessage && (
                    <div
                      className={`mt-3 text-sm ${
                        promoApplied ? "text-emerald-600" : "text-rose-600"
                      }`}
                    >
                      {promoMessage}
                    </div>
              ...
              <div className="lg:text-right">
                ...
                <div className="mt-2 text-3xl font-extrabold text-[#5A57FF]">
                  {total} DA
                </div>
                {savings > 0 && (
